...
SECRET_PATTERNS = [
    (re.compile(r"\b(?:sk|rk|pk)-[A-Za-z0-9_-]{16,}\b"), "<已脱敏-令牌>"),
    (re.compile(r"\bgh[pousr]_[A-Za-z0-9]{20,}\b"), "<已脱敏-GitHub-令牌>"),
    (re.compile(r"(?i)\bBearer\s+[A-Za-z0-9._~+/-]{12,}={0,2}"), "Bearer <已脱敏>"),
    (re.compile(r"(?i)\b(api[_-]?key|access[_-]?token|refresh[_-]?token|password)\s*[:=]\s*[^\s,;]+"), r"\1=<已脱敏>"),
    (re.compile(r"\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b", re.I), "<已脱敏-邮箱>"),
    (re.compile(r"(?<!\w)/(?:Users|home)/[^\s,;:)\]}>]+"), "<已脱敏-本地路径>"),
    (re.compile(r"(?i)\b[A-Z]:\\\\Users\\\\[^\s,;:)\]}>]+"), "<已脱敏-本地路径>"),
]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--window", default="7d", help="相对时间窗，例如 24h、7d、2w 或 all")
    parser.add_argument("--since", help="ISO 格式的起始日期或时间，包含边界")
    parser.add_argument("--until", help="ISO 格式的结束日期或时间，不包含边界")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="包含真实会话标识、本地路径、客户端元数据和工作目录过滤值",
    )
...
def resolve_bounds(args: argparse.Namespace) -> tuple[datetime, datetime]:
    ...
    if args.since:
        ...
        if not match:
            raise ValueError("--window 格式应为 24h、7d 或 2w")
...
def session_record(path: Path, meta: dict[str, Any], args: argparse.Namespace) -> dict[str, Any] | None:
    ...
    title = chosen[0][:140] if chosen else "（没有实质用户消息）"
    record = {
...
def collect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not args.session_root.exists():
        raise FileNotFoundError(f"找不到会话根目录：{args.session_root}")
...
def markdown(data: dict[str, Any]) -> str:
    ...
    lines = [
        "# Codex 会话清单",
        "",
        f"- 时间范围：`{data['scope']['since']}` 至 `{data['scope']['until']}`",
        f"- 用户根会话：{counts['root_user_sessions']}",
        f"- 返回的实质会话：{counts['returned_sessions']}",
        f"- 已排除技术探针：{counts['probe_sessions_excluded']}",
        f"- 子会话：{counts['subagent_or_child_sessions']}",
        "",
        "| 时间（UTC） | 会话 | 主题 | 消息数 |",
        "|---|---|---|---:|",
    ]
...
def main() -> int:
    ...
    if args.limit <= 0 or args.max_messages <= 0 or args.max_chars < 80:
        print("limit 与 max-messages 必须为正数，max-chars 必须大于或等于 80", file=sys.stderr)
        return 2
    try:
        data = collect(args)
